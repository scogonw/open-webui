...
<div
	class="w-full flex p-2 items-center hover:bg-[#F3F6FD] cursor-pointer"
	on:click={() => goto(`/knowledge/${file?.resource_id}`)}
>
	<div class="w-12"><SmallFolder /></div>
	<div class="w-72 flex-grow text-sm text-[#272833] font-semibold dark:text-white cursor-pointer truncate">
		{file?.title}
	</div>
